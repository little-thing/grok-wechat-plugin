...
2. **自动建好连接器与例行任务**  
   助手会加上微信连接器（所有助手都能用），并创建 Routine **「微信入站唤醒」**（Webhook）和 **「微信监听保活」**（每 5 分钟只拉监听进程）。右侧「例行任务」列表里能看到它们。

3. **扫码登录微信**  
   对话里会出一张二维码，用手机微信扫码确认绑定。需要绑定更多微信账号时，再次取码扫码即可，已有账号保持在线。
...
一个连接器可绑定多个微信个人号。每人独立扫码，互不影响；`wechat_status` 查看全部账号。入站消息带 `ilink_bot_id`，回复时用同一账号的 `wechat_send`。
